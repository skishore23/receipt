...
const FACTORY_CHAT_LOOP_TEMPLATE = [
  ...
  "- If the bound objective is blocked and the user wants to continue that same work, use `action:\"react\"` with a `note`; the runtime will resume the blocked objective in place.",
  "- If the bound objective is completed, canceled, or failed and the user wants fresh work, use `action:\"react\"` with a `note` or `prompt`; the runtime will create a follow-up objective and rebind the thread.",
  ...
  "- When `factory.output` returns `handoffPhase: \"evidence_ready\"` with `evidenceContents`, the controller has concluded evidence collection. Inspect the evidence, wait for the bounded synthesis pass if it is still running, or present the findings if the objective is already complete.",
  "- When `factory.output` returns `handoffPhase: \"synthesizing\"`, do not steer the child. The controller has already moved the task into its bounded synthesis phase; wait for that pass or inspect its latest output.",
  "- Never tell the user to `cat` a file path, reference an artifact path, or say 'the worker found X at path Y'. When `evidenceContents` is available, absorb the data and answer the user's question directly as the profile would.",
  "- The `evidenceContents` field is raw material for you, not a final answer. Restructure, summarize, or expand it based on what the user asked and how the profile speaks.",
  ...
  "- if data or evidence matters, present it clearly in markdown instead of raw JSON or file paths",
  "- if `evidenceContents` was returned by `factory.output`, use it as source material — do not paste it verbatim; reshape it through the profile's voice for the user",
