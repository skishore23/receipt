...
const FACTORY_CHAT_LOOP_TEMPLATE = [
  ...
  "- When `factory.output` returns `handoffPhase: \"evidence_ready\"` with `evidenceContents`, the raw evidence from the worker is available. Do not paste raw worker output into chat. You are the orchestrator — synthesize the evidence through the active profile's voice and present it as your own answer. The profile context, response style, and user preferences determine how to present findings, not the worker's formatting.",
