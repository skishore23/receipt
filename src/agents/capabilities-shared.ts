...
export const capabilityInput = {
  ...
  sessionSearch: z.object({
    ...
    sessionStream: z.string().optional(),
    limit: z.number().finite().optional(),
  }).passthrough(),
  sessionRead: z.object({
...
export const capabilityDescriptions = {
  ...
  "session.search": '{"query": string, "repoKey"?: string, "profileId"?: string, "sessionStream"?: string, "limit"?: number} — Search prior chat transcript messages with FTS-backed recall.',
  "session.read": '{"target"?: string, "limit"?: number} — Read chronological transcript messages for the current chat session or a specific chat/session stream.',
  ...
  "agent.inspect": '{"file": string, "maxChars"?: number} — Read another agent\'s event history by stream id such as agents/factory/<repoKey>/<profileId>.',
  ...
  "profile.handoff": '{"profileId": string, "reason": string, "goal": string, "currentState": string, "doneWhen": string, "evidence"?: string[], "blockers"?: string[], "objectiveId"?: string, "chatId"?: string} — Hand the conversation or objective off to another allowed Factory profile with a structured engineer handoff and queue the continuation explicitly.',
  "job.control": '{"jobId": string, "command": "abort", "reason"?: string} — Abort a running child job.',
  "factory.dispatch": '{"action"?: "create"|"react"|"promote"|"cancel"|"cleanup"|"archive", "objectiveId"?: string, "prompt"?: string, "note"?: string, "title"?: string, "baseHash"?: string, "objectiveMode"?: "delivery"|"investigation", "severity"?: 1|2|3|4|5, "checks"?: string[], "channel"?: string, "reason"?: string} — Create or operate on a tracked Factory objective. With a bound/current objective, omit `action` to continue it. Use `note` for follow-up/react work and `prompt` for create. If the bound objective is blocked, `action:"react"` resumes it in place. If the bound objective is terminal, `action:"react"` plus a note/prompt creates a follow-up objective and rebinds the thread.',
