...
const resolveParseTarget = async (
  ...
  if (!trimmed || trimmed === "latest") {
    ...
    const fallback = latestObjective ?? pickLatest(entries);
    if (!fallback) throw new Error(`No receipt streams found under ${dataDir}`);
    return {
      kind: streamKind(fallback.stream) ?? "objective",
      id: fallback.stream.replace(/^.*\//u, ""),
