...
import {
  ...
  objectiveIdForJob,
  profileIdForJob,
  shouldPublishProfileBoardForJobChange,
} from "./job-metadata";
...
const serverConfig = await resolveServerConfig(process.cwd());
const {
  port: PORT,
  factoryRuntime: FACTORY_RUNTIME,
  ...
  factoryChatModel: FACTORY_CHAT_MODEL,
  jobStream: JOB_STREAM,
  jobWorkerId,
  chatJobConcurrency,
  orchestrationJobConcurrency,
  codexJobConcurrency,
  jobIdleResyncMs,
  jobLeaseMs,
  codexJobLeaseMs,
  ...
  onJobChange: async (jobs) => {
    for (const job of jobs) {
      ...
      const jobMetadata = job
        ...
        : undefined;
      const objectiveId = objectiveIdForJob(jobMetadata);
      if (objectiveId) {
        sse.publish("factory", objectiveId);
        sse.publish("objective-runtime", objectiveId);
      }
      const profileId = profileIdForJob(jobMetadata);
      if (profileId && shouldPublishProfileBoardForJobChange(jobMetadata)) {
        sse.publish("profile-board", profileId);
      }
...
const { service: factoryService } = createFactoryServiceRuntime({
  ...
  repoSlotConcurrency: FACTORY_RUNTIME.repoSlotConcurrency,
  memoryTools,
...
const ASSET_DIR = resolveAssetDir(import.meta.dir);
