...
import { resolveFactoryRuntimeConfig, type FactoryRuntimeConfig } from "../factory-cli/config";
import { deriveServerRuntimeFlags, type ServerJobBackend } from "./http-topology";
...
export const resolveServerConfig = async (cwd: string): Promise<ReceiptServerConfig> => {
  ...
  const processRole = jobBackend === "resonate" && !requestedProcessRole
    ? "api"
  ...
  return {
    ...
    factoryChatModel: process.env.RECEIPT_FACTORY_CHAT_MODEL?.trim()
      || process.env.OPENAI_MODEL
      || "gpt-5.4-mini",
    jobStream: "jobs",
    jobWorkerId: process.env.JOB_WORKER_ID ?? `worker_${process.pid}`,
    chatJobConcurrency: parseWorkerConcurrency(process.env.CHAT_JOB_CONCURRENCY, 50),
    orchestrationJobConcurrency: parseWorkerConcurrency(process.env.ORCHESTRATION_JOB_CONCURRENCY, 20),
    codexJobConcurrency: parseWorkerConcurrency(process.env.CODEX_JOB_CONCURRENCY, 30),
    jobIdleResyncMs: clampIntegerEnv(process.env.JOB_IDLE_RESYNC_MS, 5_000, { min: 1_000 }),
    jobLeaseMs: clampIntegerEnv(process.env.JOB_LEASE_MS, 300_000, { min: 1_000 }),
    codexJobLeaseMs: clampIntegerEnv(process.env.CODEX_JOB_LEASE_MS, 900_000, { min: 1_000 }),
