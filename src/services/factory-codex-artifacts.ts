...
export const buildFactoryMemoryScriptSource = (configPath: string): string => {
  ...
  return [
    ...
    `  const contextSummaryPath = resolvePacketPath(config.contextSummaryPath);`,
    `  const summaryText = contextSummaryPath && fs.existsSync(contextSummaryPath)`,
    `    ? fs.readFileSync(contextSummaryPath, "utf-8").trim()`,
    `    : "";`,
    ...
    `  if (!pack) {`,
    `    process.stdout.write(summaryText ? compact(summaryText, maxChars) + "\\n" : "No recursive context pack found.\\n");`,
    `    process.exit(0);`,
    `  }`,
    `  const lines = [`,
    ...
    `    ...(pack.candidateLineage || []).map((candidate) => \`- \${candidate.candidateId} [\${candidate.status}]\${candidate.summary ? \` · \${candidate.summary}\` : ""}\${candidate.handoff && candidate.handoff !== candidate.summary ? \` · handoff \${candidate.handoff}\` : ""}\`),`,
