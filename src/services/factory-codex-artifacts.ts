...
export const buildFactoryMemoryScriptSource = (configPath: string): string => {
  ...
  return [
    ...
    `    pack.infrastructureKnowledge?.roleLabel ? \`Infrastructure role: \${pack.infrastructureKnowledge.roleLabel}\` : "",`,
    `    pack.infrastructureKnowledge?.selectedEntries?.length ? "Reusable infrastructure knowledge:" : "",`,
    `    ...(pack.infrastructureKnowledge?.selectedEntries || []).map((entry) => \`- \${entry.summary}\${entry.accountId ? \` [account \${entry.accountId}]\` : ""}\${entry.knowledgePath ? \` | \${entry.knowledgePath}\` : ""}\`),`,
