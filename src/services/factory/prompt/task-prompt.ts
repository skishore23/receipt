...
const renderTaskPromptBody = (input: {
  ...
  return [
    ...
    ``,
    `## Objective Alignment`,
    `Before marking the task complete, attach a structured objective alignment report as artifacts/objective_alignment.json with:`,
    `- the objective statement`,
    `- what changed, including files touched and any PR or commit reference`,
    `- verification steps actually performed`,
    `- links to evidence, including screenshots or DOM snapshots when applicable`,
    `- a checklist item confirming the report exists before completion`,
    `If the report is missing, treat the task as incomplete or changes_requested rather than approved.`,
    ``,
