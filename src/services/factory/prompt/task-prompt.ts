...
const renderTaskPromptBody = (input: {
  ...
  return [
    ...
    input.state.objectiveMode === "investigation"
      ...
      : `{ "outcome": "approved" | "changes_requested" | "blocked" | "partial", "summary": string, "handoff": string, "artifacts": [{ "label": string, "path": string | null, "summary": string | null }], "scriptsRun": [{ "command": string, "summary": string | null, "status": "ok" | "warning" | "error" | null }], "completion": { "changed": string[], "proof": string[], "remaining": string[] }, "structuredEvidence": { "proofBundle": { "logs": string[], "artifacts": string[] }, "alignment": { "status": "not_performed", "reason": string } }, "alignment": { "verdict": "aligned" | "uncertain" | "drifted", "satisfied": string[], "missing": string[], "outOfScope": string[], "rationale": string }, "nextAction": string | null }`,
