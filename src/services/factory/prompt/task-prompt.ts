...
const renderTaskPromptBody = (input: {
  ...
  return [
    ...
    input.state.objectiveMode === "investigation"
      ...
      : `{ "outcome": "approved" | "changes_requested" | "blocked" | "partial", "summary": string, "handoff": string, "artifacts": [{ "label": string, "path": string | null, "summary": string | null }], "scriptsRun": [{ "command": string, "summary": string | null, "status": "ok" | "warning" | "error" | null }], "structuredEvidence": [{ "title": string, "summary": string, "detail": string | null }], "completion": { "changed": string[], "proof": string[], "remaining": string[] }, "alignment": { "verdict": "aligned" | "uncertain" | "drifted", "satisfied": string[], "missing": string[], "outOfScope": string[], "rationale": string }, "nextAction": string | null }`,
    ...
    input.state.objectiveMode === "investigation"
      ...
      : `For delivery tasks, keep the envelope small. Always include an explicit handoff string, scriptsRun, structuredEvidence, completion, and alignment. Use the alignment block to map the result back to the objective contract before you return final JSON.`,
    ``,
    `## Starting Hint`,
