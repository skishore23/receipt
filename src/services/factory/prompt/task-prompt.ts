...
const renderTaskPromptBody = (input: {
  ...
  return [
    ...
    `Delivery helper APIs: use reportScriptRun(cmd, exitCode, logsRef) for every validation or evidence command and attachEvidence(...) for each structured evidence item before you complete the task.`,
    ``,
    ...
    input.state.objectiveMode === "investigation"
      ...
      : `{ "outcome": "approved" | "changes_requested" | "blocked" | "partial", "summary": string, "handoff": string, "artifacts": [{ "label": string, "path": string | null, "summary": string | null }], "scriptsRun": [{ "command": string, "summary": string | null, "status": "ok" | "warning" | "error" | null }], "structuredEvidence": { "type": string, "uri": string | null, "inline": string | null, "summary": string }, "alignmentReport": { "goals": string[], "checks": string[], "verdict": "aligned" | "uncertain" | "drifted" }, "completion": { "changed": string[], "proof": string[], "remaining": string[] }, "alignment": { "verdict": "aligned" | "uncertain" | "drifted", "satisfied": string[], "missing": string[], "outOfScope": string[], "rationale": string }, "nextAction": string | null }`,
    ...
    input.state.objectiveMode === "investigation"
      ...
      : `For delivery tasks, keep the envelope small. Always include an explicit handoff string, scriptsRun, structuredEvidence, alignmentReport, completion, and alignment. Use the alignment block to map the result back to the objective contract before you return final JSON.`,
    ``,
    `## Starting Hint`,
